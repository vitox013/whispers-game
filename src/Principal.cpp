...
Principal::Principal() : pGraphic(pGraphic->getGraphicManager()), characters() {
    ...
    Character::Player* player =
        new Character::Player(Vector2f(50.0f, 0.0f), Vector2f(50.0f, 50.0f));
    Character::Enemy::Enemy* enemy = new Character::Enemy::Enemy(
        Vector2f(50.0f, 200.0f), Vector2f(50.0f, 50.0f), player);

    Character::Character* p2 = static_cast<Character::Character*>(enemy);
    Character::Character* p1 = static_cast<Character::Character*>(player);
...
void Principal::execute() {
    while (pGraphic->isWindowOpen()) {
        ...
        for (int i = 0; i < (int)characters.size(); i++) {
            characters[i]->move();
            Collider.ColissionCheck(characters);
